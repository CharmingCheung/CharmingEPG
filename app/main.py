...
async def request_skygonz_epg():
    """Update SkyGo NZ EPG data"""
    platform = "skygonz"
    logger.info(f"📺 正在更新平台EPG数据: {platform}")
    ...
    try:
        ...
        channels, programs = await get_skygonz_epg()
        if not channels:
            logger.warning(f"⚠️ 未找到{platform}的频道数据")
            return
...
async def request_telus_epg():
    """Update Telus EPG data"""
    platform = "telus"
    logger.info(f"📺 正在更新平台EPG数据: {platform}")
    ...
    try:
        ...
        channels, programs = await get_telus_epg()
        if not channels:
            logger.warning(f"⚠️ 未找到{platform}的频道数据")
            return
...
async def request_bein_epg():
    """Update beIN SPORTS EPG data for all regions with available schedules."""
    platform = "bein"
    logger.info(f"📺 正在更新平台EPG数据: {platform}")
